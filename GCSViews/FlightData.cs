...
namespace MissionPlanner.GCSViews
{
    public partial class FlightData : MyUserControl, IActivate, IDeactivate
    {
        ...
        private void setQuickViewRowsCols(int cols, int rows)
        {
            tableLayoutPanelQuick.PerformLayout();
            tableLayoutPanelQuick.SuspendLayout();
            tableLayoutPanelQuick.ColumnCount = Math.Max(1, cols);
            tableLayoutPanelQuick.RowCount = Math.Max(1, rows);
            quickViewRight.PerformLayout();
            quickViewRight.SuspendLayout();
            quickViewRight.ColumnCount = Math.Max(1, cols);
            quickViewRight.RowCount = Math.Max(1, rows);
            ...
            // add extra
            int qvCount;
            for (qvCount = tableLayoutPanelQuick.Controls.Count; total / 2 > qvCount; qvCount++)
            {
        ...
        private void fixQvtableLayout()
        {
            ...
            tableLayoutPanelQuick.ColumnStyles[0].SizeType = SizeType.Percent;
            tableLayoutPanelQuick.ColumnStyles[0].Width = 100.0f / tableLayoutPanelQuick.ColumnCount;
            quickViewRight.ColumnStyles[0].SizeType = SizeType.Percent;
            quickViewRight.ColumnStyles[0].Width = 100.0f / quickViewRight.ColumnCount;
        ...
        private void loadQVSetting(int qvId)
        {
            Control[] ctls = Controls.Find("quickView" + qvId, true);
            if (ctls.Length > 0)
            {
                QuickView QV = (QuickView)ctls[0];

                // set description and unit
                string desc = Settings.Instance["quickView" + qvId];
                if (QV.Tag == null)
                    QV.Tag = desc;
                ...
                try
                {
                    var b = new Binding("number", bindingSourceHud,
                        Settings.Instance["quickView" + qvId], true);
        ...
        void chk_box_quickview_CheckedChanged(object sender, EventArgs e)
        {
            ...
            if (checkbox.Checked)
            {
                ...
                var b = new Binding("number", bindingSourceHud, checkbox.Name,
                    true);
                b.Format += new ConvertEventHandler(BindingTypeToNumber);
                b.Parse += new ConvertEventHandler(NumberToBindingType);
        ...
        private void setViewCountToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (InputBox.Show("Rows", "Enter number of rows to have.", ref rows) == DialogResult.OK)
            {
                if (rows.IsNumber() && cols.IsNumber())
                    ...
                    setQuickViewRowsCols(int.Parse(cols.ToString()), int.Parse(rows.ToString()));
        ...
        private string hudMsgHandler(string errorCode)
        {
            switch (errorCode)
            {
                ...
                case "E9":
                    return "پهپاد AHRS خطا در";

                case "E10":
                    return "رادیو کنترل قطع می باشد";
